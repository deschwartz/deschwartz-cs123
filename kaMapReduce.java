...
public class kaMapReduce {
	...
	public class logDateComparator implements Comparator<String> {
	...
	// of type Text.
		...
		public void reduce(Text key, Iterable<Text> values,
			...
			for (Text val : values) {
				...
				sorted_tree.add(val.toString());
			}
		
			// extract the data I want
			String earliest = sorted_logs.first()[TIMESTAMP];
			String latest = sorted_logs.last()[TIMESTAMP];
		
			iterator<String> logs_iterator = sorted_logs.iterator();
		
			for (String log : logs_iterator) {
				// log_arr
				log_arr = log.split(",");
			...
			String data = StringUtils.join(num_correct, ",") + "," + 
						  ...
						  StringUtils.join(total_timetaken / 1000, ",") + "," + 
